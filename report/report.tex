...
\textbf{Classifier:} OneVsRestClassifier wrapping SGDClassifier with log-loss, L2 regularization, and balanced class weights. Global threshold sweep on validation set $\rightarrow$ $t^* = 0.53$.
...
\begin{itemize}[nosep,leftmargin=1.5em]
    \item {Phase 1 - frozen :} 5 epochs at $\text{lr} = 10^{-3}$; trains attention + classifier only.
    \item {Phase 2  - fine-tune:} 7 epochs at $\text{lr} = 2 \times 10^{-5}$ with last 2 BERT layers unfrozen.
\end{itemize}
